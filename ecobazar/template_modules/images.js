					...
					for (const file of files) {
						...
						if (file.endsWith('.html') || file.endsWith('.js')) {
							...
							content = await replaceAsync(content, /<a\s[^>]*href=["']([^"']+\.(jpg|jpeg|avif|png|gif|webp))["'][^>]*>/gi, async (data, url) => {
								return await returnUrl(data, url, uniqImages)
							})
...
// Повернення шляху
async function returnUrl(data, url, uniqImages) {
	let inset
	if (url.startsWith('../')) {
		url = url.replace('../', '')
		inset = true
	...
	if (fs.existsSync(`src/${url}`)) {
		uniqImages.add(url)
		let imageLine = await imageResizeInit(`src/${url}`, [], [], url.split('.').pop(), null, 'url')
