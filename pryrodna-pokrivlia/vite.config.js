...
export default defineConfig(({ command, mode, ssrBuild }) => {
	if (!isWpBlocks) {
		return {
							...
							if (isHost) {
								setTimeout(() => {
									...
									for (const key in urls) {
										...
										if (key === 'local') {
											logger(`_DEV_HOST_ADDRESS`, isWp ? `http://localhost:8080` : element[0])
										} else {
			...
			build: {
				outDir: isWp ? path.join(__dirname, "src/wordpress/fls-theme/build") : path.join(__dirname, "dist"),
				emptyOutDir: true,
				manifest: false,
				...
				assetsInlineLimit: 0,
				rollupOptions: {
					input: isWp ? ['src/wordpress/fls-theme/assets/app.js'] : globSync('./src/*.html', { ignore: [`./src/${templateConfig.devcomponents.filename}`] }),
					plugins: [
		...
		return {
			...
			build: {
				outDir: path.join(__dirname, "src/wordpress/fls-theme/components/blocks/admin/dist"),
				emptyOutDir: true,
				cssMinify: false,
				rollupOptions: {
					input: ['src/wordpress/fls-theme/assets/app.js'],
					output: [{
